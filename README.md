...
| [`cleanup`](cleanup/SKILL.md) | Post-merge tidy-up: prunes dead branches and leftover files, then corrects any documentation the work has left misleading. |
...
`review-with-me` resolves what to review in order: a PR/MR you named, the open PR/MR for the current branch, then the branch diff against the default branch. It uses whichever CLI matches the remote — `gh` for GitHub, `glab` for GitLab.

`cleanup` runs at the other end of that workflow, once the PR/MR is merged in the browser. Branch deletion is deliberately conservative — it uses `git branch -d` and lets it refuse — and documentation edits always stop for approval before being committed.
...
```sh
git clone git@github.com:dantebarr/agent-skills.git ~/workspace/agent-skills
...
Start Claude Code and type `/` — `review-with-me` and `cleanup` should appear in the list.
